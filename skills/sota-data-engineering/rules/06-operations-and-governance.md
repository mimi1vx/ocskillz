...
(RBAC and row-policy mechanics are engine-specific; PII classification and
lawful basis live in `sota-privacy-compliance`. This is the
data-platform layer.)
...
- **Crypto-shredding** (per-subject encryption keys; erase the key to erase
  ...
  rewriting is infeasible — rigorous key management is required.
