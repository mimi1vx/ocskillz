...
description: >-
  ...
  threat hunting. Triggers: logging, metrics, tracing,
...
Read repository instructions and preserve its telemetry stack. This skill owns
generic logs, metrics, traces, SLOs, alerts, and operational readiness; domain
skills own ML-, LLM-, and data-specific signals.
...
7. **Match the operational surface to scope and risk**: when the feature adds a
   new failure mode or SLO, update the relevant health signal, dashboard,
   alert, and runbook. Do not scaffold unrelated infrastructure for a focused
   internal change.
