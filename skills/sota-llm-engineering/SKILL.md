...
Read repository instructions and preserve its established SDK and toolchain.
This skill owns LLM quality, evals, prompts, RAG, orchestration, and model
operations. `sota-code-security` owns prompt-injection and tool authorization,
`sota-sandboxing` owns execution isolation, `sota-privacy-compliance` owns PII
policy, and `sota-observability` owns generic telemetry foundations.
...
provider reference skill before encoding it. Write code
