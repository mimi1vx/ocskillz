...
Inspect `AGENTS.md`, `pyproject.toml`, lockfiles, CI, and supported Python
versions before proposing changes. For new projects and explicit modernization
work, strongly prefer **uv** for environments, dependencies, locking, and
commands; **Ruff** for linting and formatting; and **ty** for type checking.
For established projects, use their effective package manager, formatter,
linter, and type checker unless the user requests migration. A different but
working toolchain is not an audit finding by itself.

This skill owns Python semantics, frameworks, and runner syntax.
`sota-testing` owns language-agnostic test strategy, doubles, test data, and
suite health. `debug-loop` owns debugging sequence, and
...
1. **Establish context first.** Check `pyproject.toml`, `uv.lock`, `.python-version`, ruff
   ...
   on a 3.10 project). For a *new* project, scaffold per rules/01 with uv, Ruff, and ty.
...
7. **Verify before declaring done:** run the project's configured checks. With the local
   defaults, use `uv run ruff check .`, `uv run ruff format --check .`, `uv run ty check`,
   and the test suite via `uv run`. Code that doesn't pass these is not done.
