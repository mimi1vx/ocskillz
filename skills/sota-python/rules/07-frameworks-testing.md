...
Framework-specific traps and Python runner mechanics. Language-agnostic test strategy,
doubles, test data, coverage philosophy, and suite health live in `sota-testing`. Deep
Python rules live elsewhere (async → rules/04, validation → rules/02, SQL safety → rules/05).
...
- Use `AsyncMock` for awaitable collaborator protocols when a simple fake is not clearer.
- Use `monkeypatch.setenv`/`delenv` and `tmp_path` so environment and filesystem changes are
  isolated and automatically cleaned up.
- For unit tests around HTTP client behavior, `responses` or the client's mock transport can
  be appropriate; integration tests should exercise the real protocol boundary.
